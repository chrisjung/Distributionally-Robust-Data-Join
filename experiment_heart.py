...
y = [1 if x == 1 else -1 for x in data.target]
...
lambda_overlapped_regularized = 0.03
...
def simulate_one_iter(X, y, iter_num):
    ...
    myLogisticRegression = logisticRegression.LogisticRegression(n_iters = 1500, learning_rate = 7e-2)
...
for i in range(total_num_sim):
    full_training_accuracy, vanilla_logistic, regularized_logistic, overlapped_acc, overlapped_regularized, acc_A = simulate_one_iter(X, y, i)
